#!/usr/bin/env python3
"""
Fixed version of chunking consistency fixer
"""
...
class ChunkingFixer:
    ...
    def split_text_into_chunks(self, text_content: str, page_number: int) -> List[Tuple[str, int]]:
        """Split text into chunks with overlap - FIXED variable name"""
        chunks = []
        
        # Jika text terlalu kecil, jangan split
        if len(text_content) <= self.max_chunk_size:
            return [(text_content, page_number)]
    ...
    async def rechunk_document(self, db, doc_id: int):
        """Re-chunk a document with consistent sizing - FIXED VERSION"""
        ...
        page_contents = {}  # Rename variable untuk hindari conflict
        for page_num, text_list in pages.items():
            page_contents[page_num] = ' '.join(text_list)
        ...
        for page_num, content in sorted(page_contents.items()):
            chunks_for_page = self.split_text_into_chunks(content, page_num)
        ...
        avg_size = sum(len(c['chunk_text']) for c in new_chunks) / len(new_chunks) if new_chunks else 0
        print(f"   ✅ Created {len(new_chunks)} chunks (avg size: {avg_size:.0f} chars)")
        
        return len(new_chunks)

async def fix_single_document(doc_id: int):
    """Fix chunking for a single document"""
    
    print(f"🔄 FIXING CHUNKING FOR DOCUMENT ID {doc_id}")
    print("=" * 60)
    ...
    async for db in get_db():
        try:
            # Analyze first
            analysis = await fixer.analyze_document(db, doc_id)
            if not analysis:
                print(f"❌ Document {doc_id} not found")
                return
            
            print(f"\n📄 Document: {analysis['kitab_name']}")
            print(f"   Pages: {analysis['total_pages']}")
            print(f"   Current chunks: {analysis['total_chunks']}")
            print(f"   Issues: {', '.join(analysis['issues'])}")
            
            # Confirm
            confirm = input(f"\n❓ Fix document {doc_id}? (yes/no): ").strip().lower()
            if confirm not in ['yes', 'y', 'ya']:
                print("❌ Operation cancelled")
                return
            
            # Re-chunk
            new_count = await fixer.rechunk_document(db, doc_id)
            
            if new_count > 0:
                print(f"\n🎉 Fixed document {doc_id}")
                print(f"   Old: {analysis['total_chunks']} chunks")
                print(f"   New: {new_count} chunks")
                
                # Verify
                new_analysis = await fixer.analyze_document(db, doc_id)
                if new_analysis and not new_analysis['issues']:
                    print("   ✅ All issues resolved")
                else:
                    print("   ⚠️  Some issues may remain")
                
                print(f"\n🔧 Next: Regenerate embeddings for document {doc_id}")
                print("   Run: python scripts/regenerate_embeddings.py --doc-ids {doc_id}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Fix chunking consistency")
    parser.add_argument("--doc-id", type=int, help="Single document ID to fix")
    parser.add_argument("--doc-ids", help="Comma-separated document IDs")
    
    args = parser.parse_args()
    
    if args.doc_id:
        await fix_single_document(args.doc_id)
    elif args.doc_ids:
        # Untuk multiple documents
        doc_ids = list(map(int, args.doc_ids.split(',')))
        
        fixer = ChunkingFixer()
        
        async for db in get_db():
            try:
                print(f"🔄 FIXING {len(doc_ids)} DOCUMENTS")
                print("=" * 60)
                
                for doc_id in doc_ids:
                    print(f"\n--- Document ID {doc_id} ---")
                    await fixer.rechunk_document(db, doc_id)
                
                print(f"\n🎉 All documents fixed")
                print(f"📋 Documents fixed: {doc_ids}")
        ...
        print("❌ Please specify --doc-id or --doc-ids")
        print("   Example: python fix_chunking_fixed.py --doc-id 1")
        print("   Example: python fix_chunking_fixed.py --doc-ids 1,8,16")
